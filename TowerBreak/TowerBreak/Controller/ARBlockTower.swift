...
class ARTowerScene: SCNScene, UIGestureRecognizerDelegate {
    ...
    func resetWorld() {
        for _ in rootNode.childNodes(passingTest: { (node, _) -> Bool in
            ...
            self.sceneView.overlaySKScene = OverlayInfoScene(size: self.sceneView.frame.size, top: "Augmented Reality Scene", line1: "Move around the room and find a flat surface", line2: "Tap the yellow zone to place the tower", bottom: "Anubhav's First AR project")
